...
class BreadcrumbManager
{
    ...
    private static function printHTML($steps)
    {
        ...
        foreach ($steps as $step) {
            if (empty($step['url'])) {
                // Página actual (Gris)
                echo '<li class="breadcrumb-item active" id="' . str_replace(' ', '-', strtolower($step['label'])) .'" aria-current="page">' . htmlspecialchars($step['label']) . '</li>';
            } else {
                // Padre (Link Naranja)
                echo '<li class="breadcrumb-item"><a id="' . str_replace(' ', '-', strtolower($step['label'])) . '" href="' . htmlspecialchars($step['url']) . '" class="text-orange">' . htmlspecialchars($step['label']) . '</a></li>';
            }
        }
